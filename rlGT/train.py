...
def train(args, Gated_net, products_price, initial_inventory, train_sequences, logger):
    ...
    for epoch in range(args.epoch_num):
        ...
        logger.info(
            "learning rate now: {},{},{}".format(optimizer.param_groups[1]['lr'], optimizer.param_groups[2]['lr'],
    ...
    for seed_ in range(args.seed_range):
        if args.test_opt:
            ...
            for seq_index in seq_indexlist:
                ...
                logger.info(
                    "opt,EIBopt mean test reward: {}, {}".format(opt_agent.total_reward, EIBopt_agent.total_reward))
        ...
        for seq_index in seq_indexlist:
            ...
            if not plot:
                if args.detail:
                    change_of_invOA, change_of_ROA, change_of_invM, change_of_RM, change_of_invE, change_of_RE, change_of_invS, change_of_RS = other_agents(
                        OA_seller, myopic_seller, E_IB_seller, sub_t_seller, LP_seller, initial_inventory, T,
                        products_price, input_sequence, args, logger, args.detail, check)
                else:
            ...
            seller_list = np.vstack((seller_list, cost))
        if args.detail:
            np.save('change/' + args.name + 'change_of_invOA_list' + str(seed_) + '.npy',
                    np.array(change_of_invOA_list))
        ...
        logger.info(
            "mean test reward: {:.4f},{:.4f},{:.4f},{:.4f},{:.4f},{:.4f}".format(mean(seller_list), mean(OA_list),
                                                                                 mean(myopic_list), mean(E_IB_list),
                                                                                 mean(sub_t_list), mean(LP_list)))
        if not plot:
